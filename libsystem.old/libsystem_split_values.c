...
{
	static char *function = "libsystem_split_values_initialize";
	...
	if( *split_values == NULL )
	{
		*split_values = memory_allocate_structure(
		...
		if( memory_set(
		     *split_values,
		     0,
		     sizeof( libsystem_split_values_t ) ) == NULL )
		{
			liberror_error_set(
			 error,
			 LIBERROR_ERROR_DOMAIN_MEMORY,
		...
		if( ( string != NULL )
		 && ( string_length > 0 ) )
		{
			( *split_values )->string = libcstring_system_string_allocate(
			                             string_length + 1 );
			...
			if( memory_copy(
			     ( *split_values )->string,
			     string,
			     sizeof( libcstring_system_character_t ) * string_length ) == NULL )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_MEMORY,
		...
		if( number_of_values > 0 )
		{
			( *split_values )->values = (libcstring_system_character_t **) memory_allocate(
			                                                                sizeof( libcstring_system_character_t * ) * number_of_values );
			...
			if( memory_set(
			     ( *split_values )->values,
			     0,
			     sizeof( libcstring_system_character_t * ) * number_of_values ) == NULL )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_MEMORY,
...
int libsystem_split_values_free(
     ...
     liberror_error_t **error )
{
	static char *function = "libsystem_split_values_free";
...
int libsystem_split_values_parse_string(
     ...
     liberror_error_t **error )
{
	libcstring_system_character_t *split_value_start = NULL;
	libcstring_system_character_t *split_value_end   = NULL;
	libcstring_system_character_t *string_end        = NULL;
	static char *function                            = "libsystem_split_values_parse_string";
	 ...
	 */
	split_value_start = (libcstring_system_character_t *) string;
	string_end        = (libcstring_system_character_t *) &( string[ string_length ] );
	...
	do
	{
		...
		while( split_value_end <= string_end )
		{
			...
			else if( *split_value_end == (libcstring_system_character_t) delimiter )
			{
				break;
			}
			split_value_end++;
	...
	if( libsystem_split_values_initialize(
	     split_values,
	     string,
	     string_length,
