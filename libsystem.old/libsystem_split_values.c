     ...
     liberror_error_t **error )
{
	...
	if( *split_values == NULL )
	{
		*split_values = (libsystem_split_values_t *) memory_allocate(
		                                              sizeof( libsystem_split_values_t ) );
		...
		if( ( string != NULL )
		 && ( string_size > 0 ) )
		{
			( *split_values )->string = (libsystem_character_t *) memory_allocate(
			                                                       sizeof( libsystem_character_t ) * string_size );
			...
			if( memory_copy(
			     ( *split_values )->string,
			     string,
			     sizeof( libsystem_character_t ) * string_size ) == NULL )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_MEMORY,
		...
		if( amount_of_values > 0 )
		{
			( *split_values )->values = (libsystem_character_t **) memory_allocate(
			                                                        sizeof( libsystem_character_t * ) * amount_of_values );
			...
			if( memory_set(
			     ( *split_values )->values,
			     0,
			     sizeof( libsystem_character_t * ) * amount_of_values ) == NULL )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_MEMORY,
     ...
     liberror_error_t **error )
{
	...
	split_value_start     = (libsystem_character_t *) string;
	split_value_end       = (libsystem_character_t *) string;
	string_end            = (libsystem_character_t *) &( string[ string_size - 1 ] );
