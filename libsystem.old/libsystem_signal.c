...
#include "libsystem_signal.h"

#if defined( WINAPI )

void (*libsystem_signal_signal_handler)( libsystem_signal_t ) = NULL;
...
{
	static char *function = "libsystem_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( libsystem_signal_signal_handler != NULL )
			{
				libsystem_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "libsystem_signal_attach";
	...
	libsystem_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     libsystem_signal_handler,
	     TRUE ) == 0 )
	{
		liberror_error_set(
		 error,
...
#if defined( _MSC_VER )
	libsystem_signal_initialize_memory_debug();
#endif
...
int libsystem_signal_detach(
     liberror_error_t **error )
{
	static char *function = "libsystem_signal_detach";
	...
	libsystem_signal_signal_handler = NULL;
