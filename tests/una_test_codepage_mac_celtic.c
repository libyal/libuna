/*
 * Library codepage_mac_celtic functions test program
 *
...
#include "../libuna/libuna_codepage_mac_celtic.h"

#if defined( __GNUC__ ) && !defined( LIBUNA_DLL_IMPORT )

/* Tests the libuna_codepage_mac_celtic_copy_from_byte_stream function
 * Returns 1 if successful or 0 if not
 */
int una_test_codepage_mac_celtic_copy_from_byte_stream(
     void )
{
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		test_values = &( una_test_codepage_mac_celtic_byte_stream_to_unicode[ test_number ] );
...
/* Tests the libuna_codepage_mac_celtic_copy_to_byte_stream function
 * Returns 1 if successful or 0 if not
 */
int una_test_codepage_mac_celtic_copy_to_byte_stream(
     void )
{
...
#endif
{
	...
	UNA_TEST_RUN(
	 "libuna_codepage_mac_celtic_copy_from_byte_stream",
	 una_test_codepage_mac_celtic_copy_from_byte_stream );

	UNA_TEST_RUN(
	 "libuna_codepage_mac_celtic_copy_to_byte_stream",
	 una_test_codepage_mac_celtic_copy_to_byte_stream );
