...
/* Tests copying an Unicode character to a byte stream
     ...
     int codepage )
{
	...
	fprintf(
	 stdout,
	 "Testing copying Unicode character to byte stream with codepage: %d\t",
	 codepage );
	...
	else if( result == 0 )
	{
		if( byte_stream_to_unicode_entry_index < number_of_byte_stream_to_unicode_entries )
		{
			fprintf(
			 stdout,
			 "Mismatch for: 0x%08" PRIx32 " expected: 0x%02" PRIx8 "",
			 ( byte_stream_to_unicode[ byte_stream_to_unicode_entry_index ] ).unicode_character,
